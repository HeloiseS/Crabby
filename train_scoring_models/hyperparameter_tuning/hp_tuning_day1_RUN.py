...
if real_or_gal == 'real':
    y_train_unbalanced_binary = y_train_unbalanced.map(lambda x: dict_type_to_preal[x])
    ...
    y_train_unbalanced_binary = y_train_unbalanced.map(lambda x: dict_type_to_pgal[x])
...
pred_TRAIN_unbalanced  = scorer.predict_proba(X_train_unbalanced).T[1]
...
plot_1Dscore(pred_TRAIN_unbalanced, y_train_unbalanced,
             label='Training (unbalanced)', color='lime', 
             ax=ax[0,0], alert_type=LABEL)
...
plt.savefig(f'hp_tuning_day1/figures/{real_or_gal}_1Dscore_LR{LR}_L2{L2}_randomsplit.png',
        bbox_inches='tight') 
...
plt.savefig(f'hp_tuning_day1/figures/{real_or_gal}_permimp_real_LR{LR}_L2{L2}_randomsplit.png')
...
print('dumping model')
dump(scorer, f'hp_tuning_day1/models/{real_or_gal}_scorer_LR{LR}_L2{L2}_randomsplit.joblib')
